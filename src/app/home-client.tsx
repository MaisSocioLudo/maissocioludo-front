...
interface HomeClientProps {
  temasSemArquivo: string[];
}

export default function HomeClient({ temasSemArquivo }: HomeClientProps) {
  const [paginaAtual, setPaginaAtual] = useState(1);

  const totalPaginas = Math.ceil(temas.length / TEMAS_POR_PAGINA);
  const temasSemArquivoSet = useMemo(
    () => new Set(temasSemArquivo),
    [temasSemArquivo]
  );
  ...
  return (
    <main className="relative overflow-hidden">
      <div className="relative mx-auto max-w-7xl px-4 py-5 sm:py-8 md:px-8 lg:px-12 lg:py-10">
        <div className="mb-8 sm:mb-12">
          <Banner />
        </div>

        <div id="sobre" className="flex flex-col gap-8 sm:gap-10">
          <section
            id="socioludo"
          ...
          >
            <div className="mb-6 flex items-center gap-3">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#C02A3A]/10 text-[#C02A3A] sm:h-12 sm:w-12">
                <PiHashBold className="text-[22px] sm:text-[24px]" />
              </div>
            ...
            <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr] lg:items-start">
              <div className="space-y-4 text-[15px] leading-7 text-zinc-700 sm:text-[16px] sm:leading-8 md:text-[17px]">
                <p>
              ...
              <div className="grid gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-1">
                <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 sm:rounded-3xl sm:p-5">
                  <p className="text-2xl font-black text-[#C02A3A] sm:text-3xl">
                    348
                  </p>
                ...
                <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 sm:rounded-3xl sm:p-5">
                  <p className="text-2xl font-black text-[var(--color-azul)] sm:text-3xl">
                    18
                  </p>
                  <p className="mt-1 text-sm font-medium text-zinc-600">
                    temas trabalhados
                ...
                <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 sm:rounded-3xl sm:p-5">
                  <p className="text-2xl font-black text-[var(--color-verde)] sm:text-3xl">
                    Offline
                  </p>
                  <p className="mt-1 text-sm font-medium text-zinc-600">
          ...
          >
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="max-w-2xl text-[15px] leading-7 text-zinc-600 sm:text-[16px] md:text-[17px]">
                  Explore os textos explicativos dos{" "}
                  <strong>{temas.length} temas</strong> disponíveis no
                  SocioLudo.
                </p>
              </div>

              <div className="w-fit rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-2 text-sm font-semibold text-zinc-600">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 sm:gap-5 lg:grid-cols-6">
              {temasPaginados.map((tema) => {
                const semArquivo = temasSemArquivoSet.has(tema.id);

                return (
                  <Link key={tema.id} href={"/estudos/" + tema.id}>
                    <div className="group">
                      <div className="overflow-hidden rounded-[18px] border border-zinc-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl sm:rounded-[24px]">
                        <div className="relative aspect-[750/1039] w-full bg-zinc-100">
                          ...
                          {semArquivo ? (
                            <div className="absolute left-2 top-2 max-w-[calc(100%-1rem)] rounded-md bg-black/80 px-2 py-1 text-[10px] font-black uppercase leading-none text-white shadow-sm sm:left-3 sm:top-3 sm:text-xs">
                              Sem arquivo
                            </div>
                          ) : null}
                        </div>
